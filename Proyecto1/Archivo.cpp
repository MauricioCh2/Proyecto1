...
bool Archivos::guardarDeportistas()
{
	...
	if (dep.good()) {
		_gym->getListaDep()->guardarDeportistas(dep);
	}
	dep.close();
	return ver;
}
...
bool Archivos::cargarDeportistas()
{
	...
	string Ssex, ShorasEn, ScanPartIron, ScanTriGana, StemP, Sestatura, SmasaMuscu, Speso, SporGrasaC;
	Fecha* fechN;
	const char* sex;
	int horasEn, canPartIron, canTriGana;
	double temP, estatura, masaMuscu, peso, porGrasaC;
	Deportista* Deportista = NULL;

	while (f_Deportistas.good()) {
		getline(f_Deportistas, cedula, '\t');
